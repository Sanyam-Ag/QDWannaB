...
namespace cli{
    ...
    public:
        ...
        std::string help() const override{ return "Returns mean of the dataset column"; }

        void execute(Context& ctx, std::vector<std::string>& args) override{
            if (args.size() != 1){
                std::cout << "Usage: mean <column>\n";
                return;
            }
            ...
            double result = core::Engine::mean(ts);

            std::cout << "Mean(" << col << ") = " << result << "\n";
        }
